...
interface StoreState {
  ...
  setSummary: (summary: { title: string; description: string }) => void;
}
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  zoomLevel: getParam("zoomLevel", 85),
  ...
  setRevision: (revisionAll: number) => {
    const { revision } = get();
    set({
      revision: revision + revisionAll,
    });
  },
  ...
  setCurrentNbPages: (currentNbPages: number) => {
    ...
    const newCurrentNbPanels =
      state.currentNbPanelsPerPage * newCurrentNumberOfPages;
    ...
    */
    ...
    set({
      ...
      previousNbPanels:
        newCurrentNbPanels > state.currentNbPanels
          ? state.currentNbPanels
          : newCurrentNbPanels < state.currentNbPanels
          ? 0
          : state.previousNbPanels,
  ...
  setCurrentNbPanels: (currentNbPanels: number) => {
    ...
    */
    ...
    set({
      ...
      // state.currentNbPanels gets copied to state.previousNbPanels
      previousNbPanels:
        currentNbPanels > state.currentNbPanels
          ? state.currentNbPanels
          : currentNbPanels < state.currentNbPanels
          ? 0
          : state.previousNbPanels,
  ...
  setShowSpeeches: (showSpeeches: boolean) => {
    ...
    try {
      localStorage.setItem(
        "AI_COMIC_FACTORY_SHOW_SPEECHES",
        `${showSpeeches || false}`
      );
    } catch (err) {
      console.error(
        `failed to persist "showSpeeches" for value "${showSpeeches}"`
      );
    }
  },
  setPanelSpeech: (newSpeech, index) => {
    const { speeches } = get();
    set({
      speeches: speeches.map((c, i) => (index === i ? newSpeech : c)),
    });
  },
  ...
  */
  ...
  convertClapToComic: async (
    ...
    const cameraShots = clap.segments.filter(
      (s) => s.category === ClapSegmentCategory.CAMERA
    );

    const shots = cameraShots
      .map((cameraShot) => ({
        camera: cameraShot,
  ...
  loadClap: async (blob: Blob) => {
    ...
    putTextInInput(
      document.getElementById(
        "top-menu-input-style-prompt"
      ) as HTMLInputElement,
      stylePrompt
    );
    putTextInInput(
      document.getElementById(
        "top-menu-input-story-prompt"
      ) as HTMLInputElement,
      storyPrompt
    );
  ...
  downloadClap: async () => {
    ...
    const cleanStylePrompt = (stylePrompt || "").replace(
      /([^a-z0-9, ]+)/gi,
      " "
    );
    ...
    const cleanName = `${cleanStoryPrompt.slice(0, 90)} (${
      cleanStylePrompt.slice(0, 90) || "default style"
    })`;
  ...
  summary: {
    title: "Your Story",
    description:
      "Unleash your creativity with AINIMATE! Transform your ideas into visually stunning comics effortlessly. Step into the world of storytelling today!",
  },
