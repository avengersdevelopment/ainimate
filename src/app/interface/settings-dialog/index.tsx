...
import ButtonBG from "@/components/shared/buttonbg";
import { Settings } from "lucide-react";

export function SettingsDialog() {
  const [isOpen, setOpen] = useState(false);
  const [renderingModelVendor, setRenderingModelVendor] =
  ...
  const [huggingfaceInferenceApiModel, setHuggingfaceInferenceApiModel] =
  ...
  const [huggingfaceInferenceApiFileType, setHuggingfaceInferenceApiFileType] =
    useLocalStorage<string>(
  ...
  const [replicateApiModelVersion, setReplicateApiModelVersion] =
    useLocalStorage<string>(
      localStorageKeys.replicateApiModelVersion,
      defaultSettings.replicateApiModelVersion
    );
  const [replicateApiModelTrigger, setReplicateApiModelTrigger] =
    useLocalStorage<string>(
  ...
  const [openaiApiLanguageModel, setOpenaiApiLanguageModel] =
    useLocalStorage<string>(
  ...
  const [groqApiLanguageModel, setGroqApiLanguageModel] =
    useLocalStorage<string>(
  ...
  const [anthropicApiLanguageModel, setAnthropicApiLanguageModel] =
    useLocalStorage<string>(
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <ButtonBG
          shadowBtn="shadow-[1px_1px_0px_#000000]"
          backgroundBtn="bg-[#F8F8F8]"
          size="icon"
        >
      ...
      <DialogContent className="w-full sm:max-w-[500px] md:max-w-[700px] bg-gray-100">
        ...
        <div className="overflow-y-scroll h-[75vh] md:h-[70vh]">
          <p className="text-base italic text-zinc-600 w-full text-center">
            ℹ️ Some models can take time to cold-start, or be under heavy
            traffic.
            <br />
          ...
          {isConfigReady && (
            <Field>
              <Label className="pt-2">
                Move the slider to set the total expected number of pages:{" "}
              ...
              <Slider
                ...
                onValueChange={(value: any) => {
                  ...
                  numericValue =
                    !isNaN(value[0]) && isFinite(value[0]) ? numericValue : 0;
          ...
          <div className={cn(`grid gap-2 pt-3 pb-1`, `text-stone-800`)}>
            ...
            <Field>
              <Label className={cn()}>
                Image generation - please choose a stable diffusion provider:
              </Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="SERVER">
                    Default Hugging Face server (free but limited capacity, not
                    always online)
                  </SelectItem>
                  <SelectItem value="HUGGINGFACE">
                    Custom Inference API model (pro hugging face account
                    recommended)
                  </SelectItem>
                  <SelectItem value="REPLICATE">
                    Custom Replicate model (will bill your own account)
                  </SelectItem>
                  <SelectItem value="OPENAI">
                    DALL·E 3 by OpenAI (partial support, will bill your own
                    account)
                  </SelectItem>
                </SelectContent>
              </Select>
              ...
              <>
                <Field>
                  <Label>
                    ...
                    recommended for higher rate limit):
                  </Label>
                  <Input
                ...
                <Field>
                  <Label>
                    The file type supported by the model (jpg, webp..):
                  </Label>
                  <Input
                    className={fonts.actionman.className}
                ...
                <p className="text-sm text-zinc-700">
                  Using a LoRA? Don&apos;t forget the trigger keyword! Also you
                  will want to use the &quot;Neutral&quot; style.
                </p>
                <Field>
            ...
            {renderingModelVendor === "OPENAI" && (
              <>
                <Field>
                  <Label>
                    OpenAI API Token (you will be billed based on OpenAI
                    pricing):
                  </Label>
                  <Input
                    className={fonts.actionman.className}
            ...
            <SectionTitle>
              👇 Story generation options (🚧 experimental feature 🚧)
            </SectionTitle>

            <p>
              ⚠️ Some vendors might be buggy or require tunning, please report
              issues to Discord.
              <br />
              ⚠️ Billing and privacy depend on your preferred vendor, so please
              exercice caution.
            </p>
            <Field>
              <Label className={cn("mt-2")}>
                Story generation - please choose a LLM provider:
              </Label>
              <Select
                onValueChange={(value: string) => {
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="GROQ">
                    Open-source models on Groq (will bill your own account)
                  </SelectItem>
                  <SelectItem value="ANTHROPIC">
                    Claude by Anthropic (will bill your own account)
                  </SelectItem>
                  <SelectItem value="OPENAI">
                    ChatGPT by OpenAI (will bill your own account)
                  </SelectItem>
                </SelectContent>
              </Select>
            </Field>
