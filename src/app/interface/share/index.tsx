...
${comicFileMd}`;
    ...
    window.open(
      `https://huggingface.co/spaces/jbilcke-hf/comic-factory/discussions/new?${paramsStr}`,
      "_blank"
    );
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        >
          Share
          <Share2 className="ml-1 h-4 w-4" color="black" />
        </ButtonBG>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4 text-stone-800">
          ...
          <p>
            👉 Step 1: Click on{" "}
            <Button onClick={handlePrint} disabled={!prompt?.length}>
              <span className="hidden md:inline">
                {remainingImages
                  ? `${allStatus.length - remainingImages}/${
                      allStatus.length
                    } panels ⌛`
                  : `Get PDF`}
              </span>
          ...
          <p>
            ...
            (Note: if you use Safari, print from the OS menu)
          </p>
          <p>
            👉 Step 3: Open your PDF and convert it to a JPG image (using
            &quot;Export to&quot; or &quot;Convert to&quot;)
          </p>
          <p>
            👉 Step 4: Click here to post:{" "}
            <Button onClick={handleShare} className="space-x-2">
