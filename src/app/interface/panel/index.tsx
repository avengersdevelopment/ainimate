...
export function Panel({
  ...
  const revisionAll = useStore((s) => s.revision);
  ...
  */
  ...
  const startImageGeneration = ({
    ...
    // console.log(
    //   `panel/index.tsx: startImageGeneration(${JSON.stringify(
    //     {
    //       prompt,
    //       width,
    //       height,
    //       nbFrames,
    //       revision: revision + panelIndex + revisionAll,
    //     },
    //     null,
    //     2
    //   )})`
    ...
    setTimeout(
      () => {
        startTransition(async () => {
          ...
          try {
            ...
            // "Failed to load the panel! Don't worry, we are retrying after 5s..")
            setTimeout(() => {
            }, longDelay);
            try {
              newRendered = await newRender({
          ...
          if (newRendered) {
            ...
            if (newRendered.status === "completed") {
              ...
              setTimeout(addSpeechBubble, 500);
      ...
      enableRateLimiter ? 1000 * panel : delay
    );
  };
  ...
  useEffect(() => {
    ...
    const renderedScene: RenderedScene | undefined =
      useStore.getState().renderedScenes[panelIndex];
    ...
    if (
      renderedScene &&
      renderedScene.status === "pregenerated" &&
      renderedScene.assetUrl
    ) {
  ...
  }, [prompt, width, height, nbFrames, revision, revisionAll, panelIndex]);
  ...
  */
  ...
  const handleReload = () => {
    ...
    setRevision(revision + panelIndex + 1 + revisionAll);
  };
  ...
  if (prompt && !rendered.assetUrl && rendered.status === "pending") {
    return (
      <div
        className={cn(
          frameClassName,
  ...
  if (prompt && !rendered.assetUrl && rendered.status === "error") {
    return (
      <div
    ...
    >
      ...
      >
        <div
          ...
          className={cn(
            `bg-stone-100 rounded-lg text-black`,
            `flex flex-row space-x-2 items-center`,
            `py-1 px-3 cursor-pointer h-[30px]`,
            `transition-all duration-200 ease-in-out`,
