...
export async function newRender({
  ...
  const nbInferenceSteps = 20 //previously 30
  const guidanceScale = 8 // previously 9
  ...
  try {
    if (renderingEngine === "OPENAI") {
      ...
      */
      ...
      // console.log("Calling huggingface Inference API, response:", res)
      ...
      if (renderingEngine === "INFERENCE_API" && huggingfaceInferenceApiModelRefinerModel) {
        try {
          ...
          // console.log("Calling huggingface Inference Refiner API, response:", res)
          ...
          console.log(`Refiner step failed, but this is not a blocker. Error details: ${JSON.stringify(err)}`)
        }
      }
    ...
    console.error(`failed to run newRender: ${err}`)
    return defaulResult
  }
}
...
export async function getRender(renderId: string, settings: Settings) {
  ...
  try {
    ...
    console.error(`failed to run getRender: ${err}`)
...
export async function upscaleImage(image: string): Promise<{
  ...
  try {
    ...
    console.error(`failed to run upscaleImage: ${err}`)
    return defaulResult
  }
}
